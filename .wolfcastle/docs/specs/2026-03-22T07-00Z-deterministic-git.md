...
- `commit_state`: include `.wolfcastle/` state in commits. Only applies when `auto_commit` is `true`. Defaults to `true`. When `false`, only code changes are committed.
- `skip_hooks`: pass `--no-verify` to git commit. Defaults to `false`. Already exists in the current config.

When `auto_commit` is `false`, the fine-grained controls are ignored.

### Staging area preservation

The current `autoCommitPartialWork` runs `git add .` which sweeps up anything the user manually staged. The new flow should preserve the user's staging area:

1. Save the current index state (`git stash --keep-index --quiet`)
2. Stage daemon changes (`git add .`)
3. Commit
4. Restore the user's index state (`git stash pop --quiet`)

If the stash/pop approach proves fragile, an alternative is to use a separate git index file (`GIT_INDEX_FILE`) for daemon commits, leaving the user's index untouched.
...
| User's staging area clobbered | Staging area preserved via stash or separate index |
