...
Add a `Warnings []string` field to `Config` (unexported or excluded from JSON via `json:"-"`):
...
```go
// In internal/config/types.go or a new warnings.go file

// Warnings field on Config struct
type Config struct {
    // ... existing fields ...
    Warnings []string `json:"-"`
}
