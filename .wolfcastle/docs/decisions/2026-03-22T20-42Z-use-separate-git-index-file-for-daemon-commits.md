...
## Consequences
- User's staging area is preserved with zero interaction
- If temp index creation fails, the daemon falls back to direct commit (clobbers staging but lands the commit)
- `git status` may show transient noise after daemon commits until the user's next git operation reconciles the index
