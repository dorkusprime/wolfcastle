...
4. **Lint/vet**: run the project's linter and static analysis. For Go projects: `go vet ./...` and `golangci-lint run ./...` (if `.golangci.yml` exists). For other languages: check the Makefile or CI config for the lint command. Fix all warnings your changes introduced. Do not suppress warnings with ignore directives unless the warning is genuinely wrong.
