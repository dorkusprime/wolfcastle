...
Prefer ES modules when the project uses `"type": "module"` in `package.json` or `.mjs` extensions. ESM is the recommended default for new projects. When CommonJS is established, stay with it. Node.js 22+ supports `require()` of synchronous ESM modules, and Node.js 23+ enables this by default, easing interop. If migration is in progress, convert leaf modules first (those with few dependents) and bridge at the edges with dynamic `import()`.
...
Prefer `--env-file` (Node.js 20.6+) for loading `.env` files without external dependencies. For projects already using `dotenv`, follow the existing pattern. Never commit `.env` files; use `.env.example` to document required variables without values.
...
Prefer the project's established test runner. The built-in `node:test` runner (stable since Node.js 20) provides `describe`/`it`, mocking via `mock.method()`, timer mocking via `mock.timers`, coverage reporting, and watch mode without external dependencies. Vitest is the standard choice for projects that use Vite or need richer assertions and snapshot testing. For projects on Jest, use `jest.fn()` and `jest.spyOn()` in the usual way.
