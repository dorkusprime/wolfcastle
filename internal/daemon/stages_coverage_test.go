...
func TestProcessInbox_IntakeStageError(t *testing.T) {
	...
	d.Config.Pipeline.Stages = []config.PipelineStage{
		{Name: "intake", Model: "nonexistent-model", PromptFile: "intake.md"},
	}
...
func TestProcessInbox_DisabledIntakeStage(t *testing.T) {
	...
	f := false
	d.Config.Pipeline.Stages = []config.PipelineStage{
		{Name: "intake", Model: "echo", PromptFile: "intake.md", Enabled: &f},
	}
...
func TestRunIntakeStage_PlanningEnabled(t *testing.T) {
	...
	_ = d.Logger.StartIteration()
	defer d.Logger.Close()
	writePromptFile(t, d.WolfcastleDir, "intake-planning.md")
...
func TestRunIntakeStage_InvocationError(t *testing.T) {
	...
	stage := config.PipelineStage{Name: "intake", Model: "bad-cmd", PromptFile: "intake.md"}
	err := d.runIntakeStage(context.Background(), stage)
	if err == nil {
...
func TestRunIntakeStage_NonZeroExitCode(t *testing.T) {
	...
	stage := config.PipelineStage{Name: "intake", Model: "fail", PromptFile: "intake.md"}
	err := d.runIntakeStage(context.Background(), stage)
	if err != nil {
...
func TestRunIntakeStage_PlanningWithOverlapMarkers(t *testing.T) {
	...
	stage := config.PipelineStage{Name: "intake", Model: "overlap-echo", PromptFile: "intake.md"}
	if err := d.runIntakeStage(context.Background(), stage); err != nil {
		t.Fatalf("intake stage error: %v", err)
	}
