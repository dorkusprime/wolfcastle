...
func TestBuildIterationContextWithDir_UsesExternalDecomp(t *testing.T) {
	...
	_ = os.WriteFile(filepath.Join(dir, "system", "base", "prompts", "decomposition.md"),
		[]byte("CUSTOM: decompose {{.NodeAddr}} now"), 0644)
	_ = os.WriteFile(filepath.Join(dir, "system", "base", "prompts", "context-headers.md"),
...
func TestBuildIterationContextWithDir_UsesExternalSummary(t *testing.T) {
	...
	_ = os.WriteFile(filepath.Join(dir, "system", "base", "prompts", "summary-required.md"),
