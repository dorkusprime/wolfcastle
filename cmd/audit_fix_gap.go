...
var auditFixGapCmd = &cobra.Command{
	Use:   "fix-gap [gap-id]",
...
func init() {
	auditFixGapCmd.Flags().String("node", "", "Target node address (required)")
	auditFixGapCmd.MarkFlagRequired("node")
	auditCmd.AddCommand(auditFixGapCmd)
}
