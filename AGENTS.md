...
- **Dependencies:** Cobra/pflag, Bubbletea v2/Lipgloss v2/Bubbles v2 (TUI), chzyer/readline, fsnotify/fsnotify, atotto/clipboard (ADR-048, ADR-101)
...
11. **Parallel mode uses scope locks.** When `daemon.parallel.enabled` is true, multiple tasks run concurrently with file-level scope locks preventing collisions. Tasks that overlap in scope yield and wait for the blocker to finish. The dispatcher writes `parallel-status.json` so `wolfcastle status` can display worker pool state. Git commits are serialized through `gitMu` with scoped file staging.
