...
Three tiers. Base, custom, local. [Higher tiers override lower ones](docs/humans/configuration.md#three-tiers). JSON objects deep-merge; arrays replace entirely; set a field to `null` to eliminate it. Configuration is not a democracy. Manage it through [`wolfcastle config`](docs/humans/cli/config-show.md) commands or edit the JSON directly.
