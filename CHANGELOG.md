...
### Features
- Interactive TUI: launching `wolfcastle` with no subcommand opens a full Bubbletea v2 terminal interface with a node tree, dashboard, log stream, node and task detail views, inbox, instance switcher, and notification toasts. The welcome screen lists running daemons alongside a directory browser for `init`. State refreshes via fsnotify with a 2-second polling fallback. (#228, #231, #232, #233, #234, #236, #237, #240)
- TUI modal overlays: inbox (`i`), log stream (`L`), and daemon start/stop (`s`) are now centered modal dialogs instead of detail-pane modes. Modals absorb all keys while open and dismiss with Esc. The daemon toggle requires Enter confirmation with contextual state display (branch, PID, draining status). (#244)
- TUI tree navigation: collapsing at the top-level root (`h`/`left`/`esc`) shows the dashboard overview in the detail pane without stealing focus from the tree. (#244)
...
- TUI welcome screen `I` actually scaffolds the project: previously it called `os.MkdirAll(".wolfcastle")` and reported success, leaving the daemon to refuse to start with "identity not configured". Now it runs the same `ScaffoldService.Init` path as the CLI. (#238)
- TUI daemon-start failures surface the real reason (captured from the child's stderr) instead of bare `exit status 1`, and appear as transient toasts instead of persistent error-bar entries that could collapse the pane layout. (#236, #237)
- TUI log stream actually streams: the filesystem watcher subsystem (~370 lines covering fsnotify, debouncing, log tailing, and a polling fallback) was constructed but never started, so the log stream view always showed "No transmissions" even when the daemon was hammering NDJSON into the log file. The watcher now delivers events through a channel wrapped in a single `WatcherMsg` envelope, the model drains it via a recursive Cmd, and both creation sites call `Start()` (with `StartPolling()` as the fallback). Real-time node-state and instance-registry updates are also restored as a side effect. (#240)
- TUI log stream binding moved from `l` to `L` so it no longer collides with the tree pane's vi-style `l = expand` binding. The footer hint `[l] logs` was actively misleading because pressing `l` in the default-focused tree pane only ever expanded the selected node and never reached the global log-stream branch. (#240)
- TUI wiring bugs found in manual smoke: search highlights are keyed by tree address (survive collapse/expand), active task shows an arrow glyph, per-node cache stays fresh across instance switches, log view loads existing content on switch, and `LatestLogFile` prefers the newest exec file over high-counter intake files. (#241)
- TUI instance-switch watcher restart was missing eager prefetch: after switching worktrees, per-leaf fsnotify subscriptions were never established for the new instance, leaving all per-task glyphs stale. (#242)
- TUI leaves added by daemon decomposition after startup now get fsnotify subscriptions and initial cache loads. Previously only leaves present at TUI launch were watched. (#243)
- TUI log and activity rendering: logs extract human-readable content from Claude API JSON envelopes instead of dumping raw JSON. System init frames are skipped. Recent Activity filters to milestones only (stage transitions, failures, blocks). All displayed timestamps converted to local time. (#243)
...
- TUI wiring smoke tests: production-realistic integration tests that exercise the full path from filesystem event to rendered output, covering search highlights, instance switching, eager prefetch, log view content loading, and `LatestLogFile` selection. (#241)
- TUI app package test coverage raised from 65% to 80%. Overall project coverage at 91%. (#244)
