...
| Commit | `{git commit SHA at archive time}` |
```

### Without Summary (opt-out)

When the summary stage is disabled in config, the entry omits the Summary section entirely. The heading order becomes: Breadcrumbs, Audit, Metadata. No placeholder text or "summary disabled" marker is inserted.
...
**Gaps found**: 1
- Missing edge case test for zero-stamina scenario.

**Fixes applied**: 1
- Added `TestFireStaminaCost_ZeroStamina` covering the edge case.

**Escalations**: None
```

When there are no gaps, fixes, or escalations, the section is still present but concise:
